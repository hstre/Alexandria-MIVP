"""
Alexandria Protocol v2 - Based on reference implementation
Fixed indentation issues and added improvements
"""
...
class Node:
    ...
    lineage: List[str] = None  # patch_ids affecting this node
...
def audit_patch(patch: Patch) -> None:
    ...
    # Category purity: payload must not smuggle category changes unless explicitly MODIFY w/ category field
    if patch.operation in {"ADD", "MODIFY"}:
        ...
        if "assumptions" in patch.payload and not isinstance(patch.payload["assumptions"], list):
...
class AlexandriaStore:
    ...
    def __init__(self):
        ...
        self.nodes: Dict[str, Node] = {}  # derived state of current branch
    ...
    def create_branch(self, new_branch_id: str, from_patch_id: Optional[str] = None) -> None:
        ...
        if from_patch_id is None:
            ...
            self._last_commit_hash[new_branch_id] = self._last_commit_hash[self.current_branch]
            self._last_timestamp[new_branch_id] = self._last_timestamp[self.current_branch]
        else:
            # branch from a specific patch (inclusive)
            idx = next((i for i, p in enumerate(base_chain) if p.patch_id == from_patch_id), None)
            if idx is None:
                raise KeyError("from_patch_id not found in current branch")
            sliced = base_chain[: idx + 1]
            self.branches[new_branch_id] = list(sliced)
            self._last_commit_hash[new_branch_id] = sliced[-1].chain["commit_hash"] if sliced else None
            self._last_timestamp[new_branch_id] = sliced[-1].timestamp if sliced else 0
    ...
    def submit(self, patch: Patch) -> str:
        ...
        if patch.timestamp <= self._last_timestamp[b]:
            raise AuditError(f"Non-monotonic timestamp for branch {b}: {patch.timestamp} <= {self._last_timestamp[b]}")
        ...
        if patch.parent_patch_id != expected_parent:
            raise AuditError(f"parent_patch_id mismatch: got {patch.parent_patch_id}, expected {expected_parent}")
    ...
    def _node_from_patch(self, patch: Patch, existing: Optional[Node]) -> Node:
        ...
        category = patch.category if patch.operation == "ADD" else (p.get("category", existing.category if existing else "EMPIRICAL"))
        ...
        new_stability = self._update_stability(prev_stability, validated=validated, decay=decay)
...
def demo():
    ...
    p2 = Patch(
        ...
        category="EMPIRICAL",
        payload={
            "content": "Material X increases energy density by 20% only below 0C; unstable above.",
